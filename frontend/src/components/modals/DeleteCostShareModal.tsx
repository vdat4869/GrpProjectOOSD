...
// Props cho modal xóa cost share
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        setError("Không thể xóa cost share");
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Không thể xóa cost share");
    } finally {
      setLoading(false);
    }
  };
    ...
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-[500px] m-4">
      <div className="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Bạn có chắc chắn muốn xóa cost share này? Hành động này không thể hoàn tác.
        </p>
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/30">
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Số tiền: ₫{costShare.totalAmount.toLocaleString()}
          </p>
        </div>
