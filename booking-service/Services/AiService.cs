...
public class AiService : IAiService
{
    ...
    public async Task<BookingSuggestionResponse?> GetBookingSuggestionAsync(BookingSuggestionRequest request)
    {
        try
        {
            var requestBody = new
            {
                ...
                co_owner_id = request.CoOwnerId,
                ownership_percentage = request.OwnershipPercentage,
                usage_history = request.UsageHistory ?? new List<Dictionary<string, object>>()
            };

            var json = JsonSerializer.Serialize(requestBody);
            var content = new StringContent(json, System.Text.Encoding.UTF8, "application/json");

            var response = await _httpClient.PostAsync("/api/ai/suggestions/booking", content);
            
            if (!response.IsSuccessStatusCode)
            {
                _logger.LogWarning("AI Service returned status {StatusCode} for booking suggestion", response.StatusCode);
                return null;
            }

            var responseContent = await response.Content.ReadAsStringAsync();
            var result = JsonSerializer.Deserialize<BookingSuggestionResponse>(responseContent, new JsonSerializerOptions
            {
                PropertyNameCaseInsensitive = true
            });

            _logger.LogInformation("Received booking suggestion from AI Service with fairness score {Score}", result?.FairnessScore);
            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error calling AI Service for booking suggestion");
            return null; // Return null to allow booking to proceed without AI suggestion
        }
    }
